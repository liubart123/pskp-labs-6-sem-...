...
//         },
//     });
const sequelize = new Sequelize('pskp_8',
    "pskp_user",
    "ZAQwsxcderfv123",
    {
        dialect: "mssql",
                ...
                if (err) {
                    ...
                    if (rtToken){
                        let legit = jwt.verify(rtToken, publicKEY, verifyRtOptions,
                            (err, payload2) => {
                                if (!err){
                                    const ttl2 = (payload2.exp - Date.now() / 1000 < 0) ? 1 :Math.round(payload2.exp - Date.now() / 1000);
...
app.post('/auth', (req, res) => {
    ...
    models.User().findOne({
        ...
        .then(dbRes => {
            ...
            let token = jwt.sign({ user_id: dbRes.dataValues.id, role:dbRes.dataValues.role }, privateKEY, signOptions)
            let rtToken = jwt.sign({ user_id: dbRes.dataValues.id,role:dbRes.dataValues.role }, privateKEY, signRtOptions)
            res.cookie(
                "token",
                token,
                {
...
app.use((req,res,next)=>{
    ...
    if (req.headers.authorization)
        ...
        token = req.signedCookies.token;
    if (!token){
        token = jwt.sign({ role:'guest' }, privateKEY, signOptions)
        res.cookie(
            "token",
            token,
            {
                ...
                if (err) {
                    ...
                    redisClient.exists(redisJwtPrefix+token,(err,exist)=>{
                        ...
                        if (exist) {
                            ...
                            let newToken = jwt.sign({ user_id: payload.user_id, role:payload.role}, privateKEY, signOptions)
                            let rtToken = jwt.sign({ user_id: payload.user_id, role:payload.role}, privateKEY, signRtOptions)
                            res.cookie(
                                "token",
                                newToken,
                                {
