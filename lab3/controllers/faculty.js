...
.then(()=>{
    console.log('faculty controller db connection was created!');
...
router.get('/', function(req, res) {
    if (req.query!=null && req.query!=undefined && req.query.id!=undefined){
        queries.selectOne('faculties',req.query.id)
        .then(answer=>{
            res.type("json");
...
router.post('/', function(req, res) {
    ...
    if(!req.body || !req.is('application/json')) 
    {
        ...
        queries.selectOne('faculties',req.body.faculty)
            .then((result)=>{
                if (result!=undefined)
            ...
            .then(()=>{
                return queries.insert("faculties",req.body)
            })
            .then(answer=>{
                res.type("json");
                res.send(JSON.stringify(answer));
...
router.delete('/', function(req, res) {
    if (req.query!=null && req.query!=undefined && req.query.id!=undefined){
        queries.destroy('faculties',req.query.id)
        .then(answer=>{
            res.type("json");
            res.status(204);
            res.send(JSON.stringify(answer));
...
router.put('/', function(req, res) {
    ...
    if(!req.body || !req.is('application/json')) 
    {
        ...
        queries.selectOne('faculties',req.body.faculty)
        queries.update("faculties",req.body)
            .then(answer=>{
                res.type("json");
                res.send(JSON.stringify(answer));
            })
