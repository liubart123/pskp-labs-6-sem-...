...
.then(()=>{
    console.log('pulpit controller db connection was created!');
    models.init(sequelize)
    // queries.selectAll('pulpits')
...
router.get('/', function(req, res) {
    if (req.query!=null && req.query!=undefined && req.query.id!=undefined){
        queries.selectOne('pulpits',req.query.id)
        .then(answer=>{
            res.type("json");
...
router.post('/', function(req, res) {
    ...
    if(!req.body || !req.is('application/json')) 
    {
        ...
        queries.selectOne('pulpits',req.body.pulpit)
            .then((result)=>{
                if (result!=undefined)
            ...
            .then(()=>{
                return queries.insert("pulpits",req.body)
            })
            .then(answer=>{
                res.type("json");
                res.send(JSON.stringify(answer));
...
router.delete('/', function(req, res) {
    if (req.query!=null && req.query!=undefined && req.query.id!=undefined){
        queries.destroy('pulpits',req.query.id)
        .then(answer=>{
            res.type("json");
            res.status(204);
            res.send(JSON.stringify(answer));
...
router.put('/', function(req, res) {
    ...
    if(!req.body || !req.is('application/json')) 
    {
        ...
        queries.selectOne('pulpits',req.body.pulpit)
        queries.update("pulpits",req.body)
            .then(answer=>{
                res.type("json");
                res.send(JSON.stringify(answer));
            })
